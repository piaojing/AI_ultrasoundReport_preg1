...
Write a complete, formal 2nd or 3rd TRIMESTER OBSTETRIC ULTRASOUND REPORT.
...
Patient details:
  ...
  LMP             : {p.get('lmp', '')}
  Indication      : {p.get('indication', 'Routine 2nd or 3rd trimester scan')}
...
def translate_report_to_vietnamese(report_text: str) -> str:
    return (
        "Translate the following 2nd or 3rd trimester obstetric ultrasound report into Vietnamese. "
...
def build_docx(english_text: str, vietnamese_text: str, patient: dict) -> Document:
    ...
    r2 = sub.add_run("Obstetric Ultrasound Report")
...
class App(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("OB-US Report Generator")
        ...
        tk.Label(
            topbar,
            text="  🔬  2nd or 3rd Trimester Ultrasound Report  |  AI-Powered",
            bg="#185FA5", fg="white",
        ...
        tk.Label(
            ref_panel, text="2nd or 3rd Trimester Reference",
            bg="#185FA5", fg="white",
        ...
        if Image and ImageTk:
            try:
                ...
                # Scrollbars must be packed BEFORE the canvas so they sit at the edges
                ref_hscroll = ttk.Scrollbar(ref_panel, orient="horizontal")
                ref_hscroll.pack(fill="x", side="bottom", padx=4, pady=(0, 4))

                ref_vscroll = ttk.Scrollbar(ref_panel, orient="vertical")
                ...
                ref_image_canvas = tk.Canvas(
                    ...
                    xscrollcommand=ref_hscroll.set,
                    yscrollcommand=ref_vscroll.set,
                )
                ...
                ref_hscroll.configure(command=ref_image_canvas.xview)
                ref_vscroll.configure(command=ref_image_canvas.yview)

                # Mouse-wheel scrolls the image when the cursor is over the ref panel
                def _on_ref_wheel(event):
                    ref_image_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")
                ref_image_canvas.bind("<MouseWheel>", _on_ref_wheel)
            except Exception as exc:
                tk.Label(
                    ref_panel,
    ...
    def _build_form(self):
        ...
        self._row(pat, "Clinical Indication",     "indication",  width=42,
                  default="Routine 2nd or 3rd trimester scan")
